...
foreach ($favoritesRecap as $recap) {
    echo '<tr>';
    echo '<td>' . $recap['stars'] . ' <img class="star" src="/img/star.png"></td>';
...
foreach ($favoritesBest as $best) {
    ...
    echo '<td>' . $best['stars_avg'] . ' <img class="star" src="/img/star.png"></td>';
    echo '</tr>';
}
echo '</tbody></table>';
echo '</div>';
...
foreach ($favorites as $fav) {
    ...
    echo '<p class="t-item"><img src="/include/resize.php?s=420&f=/img/cars/' . $pic_scr . '"></p>';
    echo '<p class="t-item col stars"">';
    echo '<span>' . $fav['stars'] . '</span>';
    // Display the number of stars, else 
    for ($i = 5; $i > 0; $i--) {
        if ($i > $fav['stars']) {
            echo '<img class="star" src="/img/star_disabled.png">';
        } else {
