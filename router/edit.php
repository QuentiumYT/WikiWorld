...
if ($editType === 'car') {
    ...
    if (!$editCar) {
        ...
        echo '<form action="/include/edit_db.php" method="POST" enctype="multipart/form-data">';
        ...
        echo '</form>';
        echo '</div>';
        backButton('/' . $editType . '/' . $editId, 'Revenir à la voiture <br>"' . $editCar['car_name'] . '"');
    }
    ...
    if (!$editConstructor) {
        ...
        echo '</form>';
        echo '</div>';
        backButton('/' . $editType . '/' . $editId, 'Revenir au contructeur <br>"' . $editConstructor['brand_name'] . '"');
    }
    ...
    if (isset($_COOKIE['player_id']) && $editId == $_COOKIE['player_id']) {
        if (!$editFav) {
            ...
            echo '<p class="t-item" style="width: 20%; margin: auto">Étoiles :&nbsp;<span id="starsCount">3</span><img class="star" src="/img/star.png"></p>';
